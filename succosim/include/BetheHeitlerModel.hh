...
#ifndef BetheHeitlerModel_h
#define BetheHeitlerModel_h 1
...
class G4Pow;
///
/// \brief The BetheHeitlerModel class класс конвертации гамма в электрон-позитронные
/// пары, используется при более низких энергиях
/// This is a class of conversion of gamma to electron-positron pair.
/// It is used for not height energy
///
class BetheHeitlerModel : public G4VEmModel
{

public:

  explicit BetheHeitlerModel(const G4ParticleDefinition* p = 0,
                               const G4String& nam = "BetheHeitler");
 
  virtual ~BetheHeitlerModel();
...
private:
  ...
  BetheHeitlerModel & operator=(const BetheHeitlerModel &right) = delete;
  BetheHeitlerModel(const  BetheHeitlerModel&) = delete;
...
inline G4double BetheHeitlerModel::ScreenFunction1(const G4double delta)
{
...
inline G4double BetheHeitlerModel::ScreenFunction2(const G4double delta)
{
  return (delta > 1.4) ? 42.038 - 8.29*G4Log(delta + 0.958)
...
inline void BetheHeitlerModel::ScreenFunction12(const G4double delta,
