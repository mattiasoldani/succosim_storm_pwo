...
#ifndef PairProductionRelModel_h
#define PairProductionRelModel_h 1
...
///
/// \brief The PairProductionRelModel class класс конвертации гамма в электрн-позитронные
/// пары, используется при более низких энергиях
/// This is a class of convertion of gamma to electron-positron pair.
/// It is used for not height energy
///
class PairProductionRelModel : public G4VEmModel
{

public:

  explicit PairProductionRelModel(const G4ParticleDefinition* p = nullptr,
                                    const G4String& nam = "BetheHeitlerLPM");
 
  virtual ~PairProductionRelModel();
  ...
  /// The corrections are provided in to another functions which are used
  /// for calculation cross section
...
protected:
  ...
  ///The correction is in this function
  ...
  G4double ComputeRelDXSectionPerAtom(G4double eplusEnergy, G4double gammaEnergy,
                                      G4double Z);
   //It is used class for calculate correction cross section
   PairProductionCorrection correction;
...
private:
  ...
  PairProductionRelModel & operator=
  (const PairProductionRelModel &right) = delete;
  PairProductionRelModel(const  PairProductionRelModel&) = delete;
...
inline 
void PairProductionRelModel::ComputePhi12(const G4double delta, G4double &phi1,
                                            G4double &phi2)
{
    if (delta > 1.4) {
...
inline G4double PairProductionRelModel::ScreenFunction1(const G4double delta)
{
...
inline G4double PairProductionRelModel::ScreenFunction2(const G4double delta)
{
  return (delta > 1.4) ? 42.038 - 8.29*G4Log(delta + 0.958)
...
inline void PairProductionRelModel::ScreenFunction12(const G4double delta,
