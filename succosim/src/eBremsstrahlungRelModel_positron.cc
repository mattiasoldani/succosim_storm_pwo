...
#include "G4Exp.hh"  // MATTIA 2024: updating G4 version

/// Определяются константы для дальнейших вычислений
const G4int eBremsstrahlungRelModel_positron::gMaxZet = 120;

// constant DCS factor: 16\alpha r_0^2/3
const G4double eBremsstrahlungRelModel_positron::gBremFactor
...
const G4double eBremsstrahlungRelModel_positron::gMigdalConstant
...
const G4double eBremsstrahlungRelModel_positron::gLPMconstant
...
const G4double eBremsstrahlungRelModel_positron::gFelLowZet  [] = {
  0.0, 5.3104, 4.7935, 4.7402, 4.7112, 4.6694, 4.6134, 4.5520
};
const G4double eBremsstrahlungRelModel_positron::gFinelLowZet[] = {
  0.0, 5.9173, 5.6125, 5.5377, 5.4728, 5.4174, 5.3688, 5.3236
};

// LPM supression functions evaluated at initialisation time
eBremsstrahlungRelModel_positron::LPMFuncs  eBremsstrahlungRelModel_positron::gLPMFuncs;

// special data structure per element i.e. per Z
std::vector<eBremsstrahlungRelModel_positron::ElementData*> eBremsstrahlungRelModel_positron::gElementData;
///
/// \brief eBremsstrahlungRelModel::eBremsstrahlungRelModel
/// \param p
/// \param nam
/// Основана на модели стандартного Geant4 и наследуется от G4VEmModel
eBremsstrahlungRelModel_positron::eBremsstrahlungRelModel_positron(const G4ParticleDefinition* p,
                                                     const G4String& nam)
  ...
  fIsLPMActive(false), fPrimaryParticle(nullptr), fIsUseCompleteScreening(false)
{
  ...
  //correction.set_axis(BremsstrahlungCorrection::PWO_001);
  correction.set_axis(BremsstrahlungCorrection::PWO_100); // added by Mattia, choose
}
/// Деструктор
eBremsstrahlungRelModel_positron::~eBremsstrahlungRelModel_positron()
{
  if (IsMaster()) {
    ...
    if (true) {  // MATTIA 2024: updating G4 version
...
///
/// \brief eBremsstrahlungRelModel::Initialise Инициализация процесса
/// \param p
/// \param cuts
/// Иницализация потокозависимая
void eBremsstrahlungRelModel_positron::Initialise(const G4ParticleDefinition* p,
                                           const G4DataVector& cuts)
{
  ...
  if (IsMaster()) {
    ...
    if (true) { InitLPMFunctions(); }  // MATTIA 2024: updating G4 version
...
void eBremsstrahlungRelModel_positron::InitialiseLocal(const G4ParticleDefinition*,
...
///
/// \brief eBremsstrahlungRelModel::SetParticle Установка начальной частицы
/// \param p
///
void eBremsstrahlungRelModel_positron::SetParticle(const G4ParticleDefinition* p)
{
  fPrimaryParticle     = p;
...
///
/// \brief eBremsstrahlungRelModel::SetupForMaterial Установка различных
/// кинематических характеристик для материала
/// \param mat материал
/// \param kineticEnergy кинитическая энергия
///
void eBremsstrahlungRelModel_positron::SetupForMaterial(const G4ParticleDefinition*,
...
///
/// \brief eBremsstrahlungRelModel::MinPrimaryEnergy Определение минимальной
/// энергии, когда взаимодействие все еще можно считать дискретным
/// \param cut
/// \return
///
G4double eBremsstrahlungRelModel_positron::MinPrimaryEnergy(const G4Material*,
...
G4double
eBremsstrahlungRelModel_positron::ComputeDEDXPerVolume(const G4Material* material,
...
G4double eBremsstrahlungRelModel_positron::ComputeBremLoss(G4double tmax)
{
...
// Computes restricted atomic cross section by numerically integrating the
// DCS between the proper kinematical limits accounting the gamma production cut
///
/// \brief eBremsstrahlungRelModel::ComputeCrossSectionPerAtom Расчет
/// сечения на атоме
/// \param p Начальная частица
/// \param kineticEnergy Кинетическая энергия
/// \param Z заряд ядра
/// \param cut энергия обрезания
/// \param maxEnergy Максимальная энергия
/// \return
///
G4double eBremsstrahlungRelModel_positron::ComputeCrossSectionPerAtom(
                                                  const G4ParticleDefinition* p,
                                                  G4double kineticEnergy,
                                                  G4double Z,
                                                  G4double,
...
///
/// \brief eBremsstrahlungRelModel::ComputeXSectionPerAtom Расчет сечения от tmin до
/// энергии частицы
///
/// \param tmin
/// \return
///
G4double eBremsstrahlungRelModel_positron::ComputeXSectionPerAtom(G4double tmin)
{
...
G4double
eBremsstrahlungRelModel_positron::ComputeRelDXSectionPerAtom(G4double gammaEnergy)
{
  G4double dxsec = 0.0;
  if (gammaEnergy < 0.) {
    return dxsec;
  ...
  dxsec =dxsec*correction.ComputeCorrection(fPrimaryKinEnergy);
  //
...
G4double
eBremsstrahlungRelModel_positron::ComputeDXSectionPerAtom(G4double gammaEnergy)
{
  G4double dxsec = 0.0;
  if (gammaEnergy < 0.) {
    return dxsec;
...
///
/// \brief eBremsstrahlungRelModel::ComputeScreeningFunctions Расчет
/// экранирующей функции
/// \param phi1
/// \param phi1m2
/// \param psi1
/// \param psi1m2
...
/// \param eps
///
void eBremsstrahlungRelModel_positron::ComputeScreeningFunctions(G4double& phi1,
                                                          G4double& phi1m2,
                                                          G4double& psi1,
                                                          G4double& psi1m2,
                                                          const G4double gam,
...
///
/// \brief eBremsstrahlungRelModel::SampleSecondaries Расчет вторичных частиц
/// \param vdp
/// \param couple
/// \param dp
/// \param cutEnergy
...
///
void
eBremsstrahlungRelModel_positron::SampleSecondaries(std::vector<G4DynamicParticle*>* vdp,
...
void eBremsstrahlungRelModel_positron::InitialiseElementData()
{
...
void eBremsstrahlungRelModel_positron::ComputeLPMfunctions(G4double& funcXiS,
                                                    G4double& funcGS,
                                                    G4double& funcPhiS,
...
void eBremsstrahlungRelModel_positron::ComputeLPMGsPhis(G4double& funcGS,
                                                 G4double& funcPhiS,
...
void eBremsstrahlungRelModel_positron::InitLPMFunctions()
{
...
void eBremsstrahlungRelModel_positron::GetLPMFunctions(G4double& lpmGs,
                                                G4double& lpmPhis,
                                                const G4double sval)
{
...
//G4double eBremsstrahlungRelModel::ComputeCorrection(G4double eKineticEnergy)
//{
