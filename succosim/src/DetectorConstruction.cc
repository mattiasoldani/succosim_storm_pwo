...
G4VPhysicalVolume* DetectorConstruction::Construct()
{
    ...
    G4double zTrackerSiliCentre1 = 11.90 * m;  // silicon tracking module 1 longitudinal centre (4.5 cm long)
    G4double zTrackerSiliCentre2 = zTrackerSiliCentre1 + 5.327 * m + (2.25 + 2.4) * cm;  // silicon tracking module 2 longitudinal centre (4.8 cm long)
    G4double zMagnet = zTrackerSiliCentre2 + 2.4 * cm + 47.6 * cm + 1 * m;  // bending magnet longitudinal centre
    G4double zTgt = zTrackerSiliCentre1 + 2.25 * cm + 48 * cm;  // target longitudinal centre
    G4double zPhCaloFront = zTrackerSiliCentre2 + 2.4 * cm + 13.425 * m;  // photon calorimeter longitudinal front
    G4double zPipe0Front = zTrackerSiliCentre0 + 2.25 * cm + 144 * cm; // pipe 0 (between tracking modules 0 & 1) longitudinal front
    G4double zPipe1Front = zTrackerSiliCentre1 + 2.25 * cm + 113 * cm; // pipe 1 (between target & tracking module 2) longitudinal front
    G4double dzPipe0 = (zTrackerSiliCentre1 - 2.25 * cm - 33 * cm) - zPipe0Front; // pipe 0 length
    G4double dzPipe1 = (zTrackerSiliCentre2 - 2.4 * cm - 24 * cm) - zPipe1Front; // pipe 1 length
    G4double xECaloShift = (5 + 5 + 0.5)*cm; // electron calorimeter shift off beam nominal path; here used to specify the shift between central lead glass block & lateral ones (not used as detectors)
    
    // target -- 0.45X0
    G4double tgtThickness = 0.4 * cm;
    G4double tgtWidth = 0.2 * cm;
    G4double tgtHeight = 5.5 * cm;
    ...
    G4LogicalVolume* caloPhLog = new G4LogicalVolume(caloPhChannelBox, leadGlass, "PhCalTest_Logical");
    G4LogicalVolume* caloPhLog_passive = new G4LogicalVolume(caloPhChannelBox, leadGlass, "PhCalTest_passive_Logical");
    caloPhLog->SetVisAttributes(brown);
    caloPhLog_passive->SetVisAttributes(brown);
	
    new G4PVPlacement(nullptr, G4ThreeVector(0, 0, zPhCaloFront + caloPhThickness / 2), caloPhLog, "PhCalTest", worldLog, false, 0);
    new G4PVPlacement(nullptr, G4ThreeVector(xECaloShift, 0, zPhCaloFront + caloPhThickness / 2), caloPhLog_passive, "PhCalTest_passive_0", worldLog, false, 0);
    new G4PVPlacement(nullptr, G4ThreeVector(0, xECaloShift, zPhCaloFront + caloPhThickness / 2), caloPhLog_passive, "PhCalTest_passive_1", worldLog, false, 0);
    new G4PVPlacement(nullptr, G4ThreeVector(xECaloShift, xECaloShift, zPhCaloFront + caloPhThickness / 2), caloPhLog_passive, "PhCalTest_passive_2", worldLog, false, 0);
